...
public:
	...
	bool TryAdd(std::function<void()>&& func) {
		...
		for (int i = 0; i < 10; ++i) {
			...
			if (tail_.compare_exchange_weak(tail, new_tail, std::memory_order_acq_rel)) {
				return true; // 成功添加
			}
		}
...
int main() {
	...
	LOG_INFO("Test completed: producers sent `, consumers processed `", g_producer_count.load(),
	         g_consumer_count.load());
